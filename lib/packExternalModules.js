...
function getOriginFromModule(module) {
  const {1: npmModulePath } = module.issuer.userRequest.split('node_modules/');
  if (!npmModulePath) return undefined;
  const scoped = npmModulePath.startsWith('@');
  const npmModuleName = scoped
    ? npmModulePath.split("/").slice(0,2).join("/")
    : npmModulePath.split("/").slice(0,1).join("/");
  ...
  _.forEach(stats.compilation.chunks, chunk => {
    ...
    chunk.forEachModule(module => {
      if (isExternalModule(module)) {
        externals.add({
          origin: getOriginFromModule(module),
